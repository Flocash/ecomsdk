			...
			btnNext.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(mChoosenPayment != null)
					{
		                ...
		                {
		                	request.getPayOption().setId(mCreateOrderResponse.getPaymentOptions().getCards().get(0).getId());
		                } else if (mCreateOrderResponse.getPaymentOptions().getMobiles().size() > 0)
		                {
		                	request.getPayOption().setId(mCreateOrderResponse.getPaymentOptions().getMobiles().get(0).getId());
		                }
